...
public class Investimento extends Conta implements CLC_Imposto {
     ...
     * vai ter no mês ja com as regras de fidelidade PTS.
    ...
    public double calRendimento() {
        ...
        if (totalPTS >= 2000) {
            totalPTS -= 2000;
            aux = (saldo * rendimentoMes()) + BOUNS_REND;
            remuneraçãoRendimento = saldo * aux;
        } else {
            remuneraçãoRendimento = saldo * rendimentoMes();
        }
        saldo = this.saldo * remuneraçãoRendimento;
